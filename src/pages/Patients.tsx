        ...
        <Card>
          <CardContent className="p-5 space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Phone className="h-3.5 w-3.5" /> {selectedPatient.phone || "No phone"}
            </div>
            {selectedPatient.notes && (
              <p className="text-sm bg-accent/50 rounded-lg p-3">{selectedPatient.notes}</p>
            )}
            <p className="text-xs text-muted-foreground">Patient since {selectedPatient.createdAt}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-medium">Appointment History</CardTitle>
          </CardHeader>
          <CardContent>
            {history.length === 0 ? (
      ...
      <div className="space-y-2">
        {filtered.map((p) => (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">{store.getAppointmentsForPatient(p.id).length} visits</span>
          </button>
        ))}
        {filtered.length === 0 && (
          <p className="text-muted-foreground text-sm text-center py-8">No patients found</p>
        )}
      </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-3 mt-2">
            ...
            <div className="space-y-1.5"><Label>Phone</Label><Input value={newPhone} onChange={(e) => setNewPhone(e.target.value)} /></div>
            <div className="space-y-1.5"><Label>Notes</Label><Textarea value={newNotes} onChange={(e) => setNewNotes(e.target.value)} rows={2} /></div>
